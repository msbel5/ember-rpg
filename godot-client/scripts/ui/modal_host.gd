...
@onready var modal_stack: TabContainer = $ModalStack
...
func _ready() -> void:
	...
	modal_stack.tabs_visible = false
...
func active_panel_id() -> String:
	if not visible:
		return ""
	var current = modal_stack.get_tab_control(modal_stack.current_tab)
	if current == null:
		return ""
...
func panel_node(panel_id: String) -> Control:
	...
	if not _panel_index_by_id.has(normalized):
		return null
	return modal_stack.get_tab_control(int(_panel_index_by_id[normalized]))
...
func show_panel(panel_id: String) -> bool:
	...
	if not _panel_index_by_id.has(normalized):
		return false
	modal_stack.current_tab = int(_panel_index_by_id[normalized])
	_title_label.text = PANEL_TITLES.get(normalized, "Panel")
	visible = true
	var current = modal_stack.get_current_tab_control()
	if current != null:
...
func _rebuild_panel_index() -> void:
	...
	for index in range(modal_stack.get_tab_count()):
		var child = modal_stack.get_tab_control(index)
		if child == null:
			continue
		...
		modal_stack.set_tab_title(index, PANEL_TITLES.get(panel_id, child.name))
