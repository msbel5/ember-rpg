...
def test_vertical_desktop_scenarios_target_semantic_dialog_travel_and_combat() -> None:
    ...
    assert next(step for step in combat.steps if step.id == "wait_for_attack_verb").action == "wait_for_node_visible"
    assert next(step for step in combat.steps if step.id == "enter_combat").node_path.endswith("FocusActionTwo")
