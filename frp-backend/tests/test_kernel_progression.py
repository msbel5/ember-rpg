...
def _class_defs() -> dict[str, ClassDef]:
    warrior_xp = [0, 2000, 4000, 8000, 16000, 32000, 64000, 125000, 250000, 500000]
    rogue_xp = [0, 1250, 2500, 5000, 10000, 20000, 40000, 70000, 110000, 160000]
    mage_xp = [0, 2500, 5000, 10000, 20000, 40000, 60000, 90000, 135000, 250000]
    priest_xp = [0, 1500, 3000, 6000, 13000, 27500, 55000, 110000, 225000, 450000]
    return {
        "warrior": ClassDef(
            class_id="warrior",
            label="Warrior",
...
def test_ac01_can_level_up_checks_exact_threshold():
    ...
    progression = ProgressionState(actor_id="a1", xp=7999, level=3, classes=["warrior"], class_levels={"warrior": 3})
...
def test_ac02_hp_gain_uses_hit_die_plus_end_modifier():
    class_defs = _class_defs()
    progression = ProgressionState(actor_id="a2", xp=8000, level=3, classes=["warrior"], class_levels={"warrior": 3})

    result = execute_level_up(progression, "warrior", class_defs["warrior"], hit_die_roll=7, end_modifier=2)
...
def test_ac03_hp_gain_has_minimum_one():
    ...
    result = execute_level_up(progression, "mage", class_defs["mage"], hit_die_roll=2, end_modifier=-1)
...
def test_ac04_bab_progression_matches_class_rates():
    ...
    assert compute_bab({"warrior": 5}, class_defs) == 5
    assert compute_bab({"rogue": 8}, class_defs) == 6


def test_ac05_save_progression_uses_good_and_poor_tables():
    class_defs = _class_defs()

    saves = compute_saves({"warrior": 6}, class_defs)
...
def test_ac06_ability_increase_only_on_every_fourth_level():
    class_defs = _class_defs()
    level_four = ProgressionState(actor_id="a4", xp=8000, level=3, classes=["warrior"], class_levels={"warrior": 3})
    level_five = ProgressionState(actor_id="a5", xp=16000, level=4, classes=["warrior"], class_levels={"warrior": 4})

    result_four = execute_level_up(level_four, "warrior", class_defs["warrior"], hit_die_roll=5, end_modifier=0)
    result_five = execute_level_up(level_five, "warrior", class_defs["warrior"], hit_die_roll=5, end_modifier=0)
...
def test_ac07_multiclass_uses_best_bab_and_saves():
    ...
    bab = compute_bab({"warrior": 5, "rogue": 3}, class_defs)
    saves = compute_saves({"warrior": 5, "rogue": 3}, class_defs)
...
def test_ac10_full_level_up_pipeline_returns_expected_values():
    class_defs = _class_defs()
    progression = ProgressionState(actor_id="a10", xp=8000, level=3, classes=["warrior"], class_levels={"warrior": 3})

    result = execute_level_up(progression, "warrior", class_defs["warrior"], hit_die_roll=6, end_modifier=2)
...
def test_ac12_progression_state_round_trip_preserves_all_fields():
    original = ProgressionState(
        ...
        class_levels={"warrior": 3, "rogue": 1},
        bab=3,
...
def test_ac13_channeler_level_up_grants_presence_based_spell_slots():
    channeler = ClassDef(
        class_id="channeler",
        label="Channeler",
        hit_die=6,
        ...
        hp_after_cap=1,
        hit_die_cap_level=10,
        xp_table=[0, 2000, 4000, 8000],
    )
    progression = ProgressionState(actor_id="a13", xp=4000, level=1, classes=["channeler"], class_levels={"channeler": 1})

    result = execute_level_up(progression, "channeler", channeler, hit_die_roll=4, end_modifier=1)

    assert result.new_level == 2
    assert result.new_spell_slots == {1: 2}
