...
| Effects and syndromes | `kernel_effects`, `kernel_systems` | DF `M06`; GemRB `M04`, `M06` | `logic-live` |
...
| Colony, jobs, and farming | `kernel_colony`, `kernel_jobs` | DF `M03`, `M04`, `M16`, `M19` | `logic-live` |
| Trade, migration, diplomacy | `kernel_store`, `kernel_world_state`, `kernel_history`, `kernel_macro_society` | DF `M11`, `M12`, `M20`; GemRB `M13`, `M14` | `logic-live` |
| Commander loop and military | `kernel_hybrid` | DF `M15`, `M18`; GemRB `M14`, `M16` | `logic-live` |
| Systems closure | `kernel_systems` | DF `M07`, `M08`, `M09`, `M10`, `M13` | `logic-live` |
...
- `frp-backend/tests/test_campaign_logic_live.py` proves campaign-runtime
  authority for effects/syndromes, colony/jobs/farming, trade/migration/
  diplomacy, and systems closure.
- `frp-backend/tests/test_command_bus.py` verifies those runtime slices survive
  real command execution.
- `frp-backend/tests/test_campaign_save_load_v2.py` verifies canonical
  persistence of the live kernel slices.
